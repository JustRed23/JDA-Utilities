...
public class LavalinkAudioLoadResultHandler extends AbstractAudioLoadResultHandler {
    ...
    public LavalinkAudioLoadResultHandler(LavalinkGuildMusicManager manager, PlaybackState prevState, String url) {
    ...
    public void ontrackLoaded(@NonNull TrackLoaded trackLoaded) {
        PlayableTrack track = fromTrack(trackLoaded.getTrack());
    ...
    public void onPlaylistLoaded(@NonNull PlaylistLoaded playlistLoaded) {
        List<PlayableTrack> tracks = playlistLoaded.getTracks().stream().map(LavalinkUtils::fromTrack).toList();
    ...
    public void onSearchResultLoaded(@NonNull SearchResult searchResult) {
        List<PlayableTrack> tracks = searchResult.getTracks().stream().map(LavalinkUtils::fromTrack).toList();
        if (!tracks.isEmpty()) {
